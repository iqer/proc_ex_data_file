...
FILE_PATH = f'上交所-mktdt00_{datetime.now().date()}.xlsx'


def valid_index_data_line(line_list):
...
def valid_first_line(line_list):
    version = line_list[1]
...
def valid_market_data_line(line_list):
...
def merge_index_data(index_data_list, line_list):
    md_stream_id = line_list[0]
    security_id = line_list[1]
    symbol = line_list[2]
    trade_volume = line_list[3]
...
def merge_stock_data(stock_data_list, line_list):
    md_stream_id = line_list[0]
    security_id = line_list[1]
    symbol = line_list[2]
    trade_volume = line_list[3]
...
def merge_bond_data(bond_data_list, line_list):
    md_stream_id = line_list[0]
    security_id = line_list[1]
    symbol = line_list[2]
    trade_volume = line_list[3]
...
def merge_fund_data(fund_data_list, line_list):
    md_stream_id = line_list[0]
    security_id = line_list[1]
    symbol = line_list[2]
    trade_volume = line_list[3]
...
def main():
    with open('./上交所数据文件/mktdt00_20230708.txt', encoding='gbk') as f:
        data = f.read()
    index_data_list = []
    stock_data_list = []
    bond_data_list = []
    ...
    data_lines = [item for item in data_lines if item]

    first_line = data_lines[0].split('|')
    last_line = data_lines[-1].split('|')
    assert first_line[0] == 'HEADER' and last_line[0] == 'TRAILER'

    tot_num = valid_first_line(first_line)
    data_lines = data_lines[1:-1]
    assert len(data_lines) == tot_num

    for line in data_lines:
        line_list = line.split('|')
        if line_list[0] == 'MD001':
            valid_index_data_line(line_list)
            line_list = [item.replace(' ', '') for item in line_list]
            merge_index_data(index_data_list, line_list)
        elif line_list[0] == 'MD002':
            valid_market_data_line(line_list)
            line_list = [item.replace(' ', '') for item in line_list]
            merge_stock_data(stock_data_list, line_list)
        elif line_list[0] == 'MD003':
            valid_market_data_line(line_list)
            line_list = [item.replace(' ', '') for item in line_list]
            merge_bond_data(bond_data_list, line_list)
        elif line_list[0] == 'MD004':
            valid_market_data_line(line_list)
            line_list = [item.replace(' ', '') for item in line_list]
            merge_fund_data(fund_data_list, line_list)

    dump_data_to_excel(index_data_list, stock_data_list, bond_data_list, fund_data_list)


def dump_data_to_excel(index_data_list, stock_data_list, bond_data_list, fund_data_list):
    index_df = pd.DataFrame(index_data_list, columns=[
        '行情数据类型',
        '产品代码',
        '产品名称',
        '成交数量',
    ...
    stock_df = pd.DataFrame(stock_data_list, columns=[
        '行情数据类型',
        '产品代码',
        '产品名称',
        '成交数量',
    ...
    bond_df = pd.DataFrame(bond_data_list, columns=[
        '行情数据类型',
        '产品代码',
        '产品名称',
        '成交数量',
    ...
    fund_df = pd.DataFrame(fund_data_list, columns=[
        '行情数据类型',
        '产品代码',
        '产品名称',
        '成交数量',
